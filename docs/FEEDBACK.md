...
| F15 | The 90/90 hip switch is the last movement with no animation | 2026-09-02 | What is left of F14. Eight of the nine gaps were closed by animations Omar generated; this one was not among them, so it is the only card in the app still reading "No preview yet". It is also the hardest of the nine to learn from cues alone, because the whole drill is a position |

_F7 to F13 all came out of the first real session, on 2026-09-02. F6 predates it and is
still Omar's to run. F15 is the one item here nobody reported — it is what is left of F14
after Omar generated the other eight animations._
...
The procedure was **[DATA_MODEL.md section 8](DATA_MODEL.md#8-starting-again-from-a-clean-slate)**:
delete `workoutSessions`, `personalRecords` and `programAssignments`, keep `profile` and
`settings`.

**That is no longer safe as written.** It was meant to be done before the first real session
and was not, so `workoutSessions` now holds a real session from 2026-09-02 as well as the
phantom. Deleting the collection would take the real one with it. What is needed instead is
the phantom document deleted by id — it is the one with the earlier `startedAt` and no
`sessionNotes` — plus any `personalRecords` whose `achievedInSessionId` points at it. Close
this item once the app stops counting a session nobody trained.
...
| F13 | A machine his gym does not have is still prescribed                                      | `feat/exercise-availability`       | "My gym has not got this machine" on the brief. It comes out of today with a reason, and from the next session on the closest equivalent is swapped in and says so. Undone in Settings                                                                               |
| F14 | Four of the seven warm-up drills have no animation                                       | `feat/generated-warmup-animations` | Omar generated the missing animations rather than hunting for a source. Eight of the nine gaps are closed — the four drills, plus thread the needle, the couch stretch, the doorway pec stretch and the rower. Only the 90/90 hip switch still says "No preview yet" |
